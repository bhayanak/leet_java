...
/**
 * <b>#3575 - Maximum Good Subtree Score</b>
 *
 * <p><b>Difficulty:</b> Hard</p>
 * <p><b>Category:</b> Array, Dynamic Programming, Bit Manipulation, Tree, Depth-First Search, Bitmask</p>
 *
 ...
 * Hint 1: Use tree dynamic programming.
 * Hint 2: Use bits (integer) to represent which digits are used.
 *
 * <h2>Approach</h2>
 * Think about the category (Array, Dynamic Programming, Bit Manipulation, Tree, Depth-First Search, Bitmask).
 *
 * @see <a href="https://leetcode.com/problems/maximum-good-subtree-score/">LeetCode #3575</a>
 */
public class MaximumGoodSubtreeScore {
    ...
    /**
     * Solves the problem: Maximum good subarray sum.
     *
    ...
    public long maximumGoodSubarraySum(int[] nums, int k) {
        long maxSum=Long.MIN_VALUE; int n=nums.length;
        for (int i=0;i<n;i++) {
            long cur=0;
            for (int j=i;j<n;j++) {
                cur+=nums[j];
                if (Math.abs(nums[i]-nums[j])<=k) maxSum=Math.max(maxSum,cur);
            }
        }
        return maxSum;
    }
    ...
    public static void main(String[] args) {
        MaximumGoodSubtreeScore sol = new MaximumGoodSubtreeScore();
        System.out.println(sol.maximumGoodSubarraySum(new int[]{1,2,3}, 0));
    }
}
