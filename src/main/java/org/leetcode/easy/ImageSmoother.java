 ...
 * @see <a href="https://leetcode.com/problems/image-smoother/">Problem</a>
 */
public class ImageSmoother {
            ...
            for (int j = 0; j < n; j++) {
                ...
                for (int di = -1; di <= 1; di++) {
                    for (int dj = -1; dj <= 1; dj++) {
                        int ni = i+di, nj = j+dj;
                        if (ni >= 0 && ni < m && nj >= 0 && nj < n) { sum += img[ni][nj]; count++; }
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(sol.imageSmoother(new int[][]{{1,2},{3,4}}));
    }
}
